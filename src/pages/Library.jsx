...
const CAT_COLORS = [
  "#34d399","#60a5fa","#f59e0b","#f87171","#fb923c",
  "#a78bfa","#e879f9","#22d3ee","#84cc16","#f43f5e",
];
      ...
      <div className="w-full max-w-md rounded-2xl flex flex-col shadow-2xl"
        ...
        <div className="px-5 py-4 border-t flex justify-end gap-3 shrink-0"
          ...
          <button onClick={onClose}
            className="px-4 py-2 rounded-lg text-[13px] font-medium text-gray-400 hover:text-white transition-colors"
...
// ── AddBookModal ──────────────────────────────────────────────────────────────
function AddBookModal({ categories, onClose, onAdd, initialData }) {
  const [form, setForm] = useState({
    ...
    currentPage: initialData
      ? Math.round((initialData.progress / 100) * (initialData.pages || 1))
      : '',
      ...
      <div className="w-full max-w-lg rounded-2xl flex flex-col shadow-2xl"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input
                ...
                placeholder="Örn: The Design of Everyday Things"
                style={inputStyle}
                autoFocus
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                maxLength="2"
                value={form.cover}
                onChange={e => update('cover', e.target.value.toUpperCase())}
                placeholder="DN"
                style={{ ...inputStyle, textAlign: 'center', fontWeight: 'bold', letterSpacing: '2px' }}
              />
            </div>
            <div>
        ...
        <div className="px-5 py-4 border-t flex justify-end gap-3 shrink-0"
          ...
          <button
            ...
            disabled={!form.title.trim() || !form.author.trim() || !form.pages}
            ...
            style={{ background: '#7c3aed' }}>
            {initialData ? 'Güncelle' : 'Kitabı Ekle'}
          </button>
        </div>
      </div>
    </div>
        ...
        >
          <div className="flex-1 overflow-hidden flex flex-col min-h-0">
            <div className="flex items-start justify-between mb-1">
              <h4 className="text-white font-semibold text-sm line-clamp-2 flex-1 pr-2">{book.title}</h4>
          ...
          <div className="flex items-center justify-between relative z-10 mt-2">
            <div className="flex gap-0.5">
              {[1, 2, 3, 4, 5].map(s => (
                <button key={s}
                  onClick={(e) => { e.stopPropagation(); onRate(book, s); }}
...
export default function Library() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const mappedBooks = (booksData || []).map(mapBackendToFrontendBook);
        ...
        if (!catsData || catsData.length === 0) {
          // Backend'de hiç kategori yok → sadece local state'e yaz
          // Kullanıcı "Kaydet" dediğinde backend'e gönderilecek
  ...
  const handleRateBook = async (book, newRating) => {
    ...
    try {
      const payload = {
        ...
        cover:       book.cover,
        colorStart:  book.color?.[0],
        colorEnd:    book.color?.[1],
        accent:      book.accent,
        spine:       book.spine,
      };
  ...
  const handleSaveBook = async (payload) => {
    try {
      const { isFavorite, tags, color, ...rest } = payload;
      ...
      if (editingBook) {
        ...
        const updated = mapBackendToFrontendBook(res.data.data || res.data);
        setBooks(prev => prev.map(b => b.id === editingBook.id ? updated : b));
      } else {
        const res = await api.post('/api/library', backendPayload);
        const created = mapBackendToFrontendBook(res.data.data || res.data);
  ...
  const toggleFavorite = async (id) => {
    const newIsFav = !favorites.has(id);
    setFavorites(prev => {
      const next = new Set(prev);
      newIsFav ? next.add(id) : next.delete(id);
      return next;
    });
    setBooks(prev => prev.map(b => b.id === id ? { ...b, isFavorite: newIsFav } : b));
    try {
      ...
      setFavorites(prev => {
        const next = new Set(prev);
        newIsFav ? next.delete(id) : next.add(id);
        return next;
      });
      setBooks(prev => prev.map(b => b.id === id ? { ...b, isFavorite: !newIsFav } : b));
    }
  ...
  const shelfGroups = categories.map(cat => ({
    ...cat,
    books: books.filter(b => b.category === cat.id),
  }));

  const totalProgress = books.length > 0
    ? Math.round(books.reduce((a, b) => a + (b.progress || 0), 0) / books.length)
    : 0;

  const statsFooter = [
    { label: "Toplam",     val: books.length },
    { label: "Okunuyor",   val: books.filter(b => b.progress > 0 && b.progress < 100).length },
    { label: "Tamamlanan", val: books.filter(b => b.progress === 100).length, color: "#34d399" },
    { label: "Başlanmadı", val: books.filter(b => b.progress === 0).length },
  ];
  ...
  return (
    <div className="flex flex-col h-screen w-full overflow-hidden"
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                {books.length} kitap · {favorites.size} favori · {books.filter(b => b.progress === 100).length} tamamlanan
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              style={{ background: "#1e1e28", color: "#9090a0", border: "1px solid #252530" }}
      ...
      <main className="flex-1 overflow-y-auto px-6 py-5">
        ...
        {!loading && filtered.length > 0 && view === "shelf" && (
          <div className="space-y-8">
            {shelfGroups.map(group => {
              const visibleBooks = group.books.filter(b => {
                if (activeCategory !== "all" && b.category !== group.id) return false;
                if (showFavOnly && !favorites.has(b.id)) return false;
                if (search) {
                  const q = search.toLowerCase();
                  if (
                ...
                <div key={group.id}>
                  ...
                  <div className="relative">
                    ...
                    <div className="h-2 rounded-b-sm mt-1"
                      style={{
                        background: "linear-gradient(180deg, #1f1a14 0%, #110d08 100%)",
          ...
          <div className="space-y-2">
            ...
            {filtered.map(book => (
              <BookCard key={book.id} book={book}
                ...
                onToggleFavorite={toggleFavorite}
                view="list"
                onEdit={(b) => { setEditingBook(b); setIsAddModalOpen(true); }}
      ...
      <footer className="flex items-center gap-6 px-6 py-3 border-t flex-shrink-0"
        ...
        {statsFooter.map((s, i) => (
          <div key={i} className="flex items-center gap-1.5">
            <div className="w-1.5 h-1.5 rounded-full" style={{ background: s.color || "#4b5563" }} />
            <span className="text-xs text-gray-500">{s.label}:</span>
            <span className="text-xs font-semibold" style={{ color: s.color || "#9ca3af" }}>{s.val}</span>
          </div>
        ))}
        <div className="ml-auto flex items-center gap-2">
          ...
          <span className="text-xs text-purple-400 font-medium">{totalProgress}%</span>
        </div>
      </footer>
